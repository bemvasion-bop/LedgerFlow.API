...
const AdminAuditLogs: React.FC = () => {
  ...
  const fetchLogs = useCallback(async (targetPage: number) => {
    ...
    try {
      const res = await api.get('/auditlogs', {
        params: { pageNumber: targetPage, pageSize: PAGE_SIZE }
      });
  ...
  }, []);  // only on mount; page changes go through handlePage
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
        <div>
          ...
          <p style={{ color: '#aaa', margin: '5px 0 0', fontSize: '0.9rem' }}>
            Read-only system activity log &mdash; {total} total entries
          </p>
        </div>
        <button className="btn btn-primary" onClick={handleRefresh} style={{ padding: '8px 18px', fontSize: '0.9rem' }}>
          Refresh
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{ color: '#aaa', fontSize: '0.9rem' }}>
          Page {page} of {totalPages || 1} &nbsp;|&nbsp; Showing {filtered.length} on this page
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: '0', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <span style={{ color: '#aaa', fontSize: '0.9rem' }}>
              {total} total entries &nbsp;&bull;&nbsp; Page {page} of {totalPages || 1}
            </span>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                ...
                style={{
                  padding: '7px 18px',
                  fontSize: '0.9rem',
                  background: page <= 1 ? 'rgba(255,255,255,0.05)' : undefined,
              ...
              <button
                ...
                style={{
                  padding: '7px 18px',
                  fontSize: '0.9rem',
                  background: page >= totalPages ? 'rgba(255,255,255,0.05)' : undefined,
